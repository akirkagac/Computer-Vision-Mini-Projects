...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        arrayPatch_mag = reshape(patch_mag,[patch_size*patch_size,1]);
        arrayPatch_theta = reshape(patch_theta,[patch_size*patch_size,1]);
        patch_angle_offset = ComputeDominantDirection(arrayPatch_mag,arrayPatch_theta);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        feature = [];
        cellPatchmag = mat2cell(patch_mag,[num_histograms num_histograms num_histograms num_histograms],...
                [num_histograms num_histograms num_histograms num_histograms]);
        cellPatchang = mat2cell(patch_theta,[num_histograms num_histograms num_histograms num_histograms],...
                [num_histograms num_histograms num_histograms num_histograms]);    
        for y = 1:num_histograms
            for x = 1:num_histograms
                arrayCellPatchmag = reshape(cellPatchmag{y,x},[pixelsPerHistogram*pixelsPerHistogram,1]);
                arrayCellPatchang = reshape(cellPatchang{y,x},[pixelsPerHistogram*pixelsPerHistogram,1]);
                [histogram,~] = ComputeGradientHistogram(8, arrayCellPatchmag, arrayCellPatchang);
                feature = [feature,histogram];
            end
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i= 1:size(gradient_magnitudes)
        k = floor((gradient_angles(i)*num_bins)/(2*pi));
        if(k ~= num_bins)
           k = k+1;
        end
        histogram(k) = histogram(k) + gradient_magnitudes(i);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [~, indexMax] = max(histogram(:));
    direction = angles(indexMax);
