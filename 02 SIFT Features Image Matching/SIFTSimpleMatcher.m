...
[N1, ~] = size(descriptor1);
[N2, d] = size(descriptor2);

for i = 1:N1
    euclideanDistance = zeros(1,N2);
    for j = 1:N2        
        for k = 1:d
           euclideanDistance(j) = euclideanDistance(j) + (descriptor1(i,k)-descriptor2(j,k))^2;
        end
        euclideanDistance(j) = sqrt(euclideanDistance(j));
        
    end
    euclideanDistanceSorted = sort(euclideanDistance);
        if euclideanDistanceSorted(1)<thresh*euclideanDistanceSorted(2)
            location2 = find(euclideanDistance == euclideanDistanceSorted(1));
            match(matchCounter,:) = [i,location2];
